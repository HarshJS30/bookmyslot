...
const httpServer = createServer((request, response) => {
  ...
  if (pathname === "/socket.io" || pathname?.startsWith("/socket.io/")) {
    // Leave Socket.IO's handshake and polling requests to Engine.IO.
    return;
  }
...
const io = new Server(httpServer, {
  cors: { origin: frontendOrigins.length === 1 ? frontendOrigins[0] : frontendOrigins },
});
...
subscriber.on("subscribe", (channel) => {
  if (channel === "realtime") subscribed = true;
});
subscriber.on("unsubscribe", (channel) => {
  if (channel === "realtime") subscribed = false;
});

void subscriber.subscribe("realtime").catch((error: unknown) => {
  subscribed = false;
  console.error("Failed to subscribe to Redis realtime channel:", error);
});
